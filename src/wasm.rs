...
fn setup<A>(mut app: A) -> Result<(), JsValue>
where
    A: App + 'static,
{
    ...
    let canvas = document
        .get_element_by_id("trowel-canvas")
    ...
    let buttons = Rc::new(RefCell::new(Buttons::empty()));
    {
        ...
        let closure = Closure::<dyn FnMut(_)>::new(move |e: web_sys::KeyboardEvent| {
            ...
            buttons.replace_with(move |buttons| match c {
                'W' => *buttons | Buttons::W,
                'A' => *buttons | Buttons::A,
                'S' => *buttons | Buttons::S,
                'D' => *buttons | Buttons::D,
    ...
    *g.borrow_mut() = Some(Closure::wrap(Box::new(move || {
        // let mut fs = WebFS::new().expect("Unable to create web fs");
